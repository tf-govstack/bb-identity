Feature: API to add new open ID connect (OIDC) clients, it can be invoked by other modules which manages the relying parties / partners.
  ...
  Scenario: The user is not able to added the new client to the Open ID Connect (OIDC), because of an invalid request
    When The user triggers an action to add the new client to the Open ID Connect (OIDC) with an invalid request
